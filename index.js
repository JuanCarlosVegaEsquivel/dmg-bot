...
// ── Damage Helpers ─────────────────────────────────────────────
function calcDamage(strength, critDmg, weaponDmg, multiplier = 1, armorBonus = 0) {
    // SkyBlock formula:
    // Initial = (5 + WeaponDMG) * (1 + Strength/100)
    // Final   = Initial * Multiplier * (1 + ArmorBonus) * (1 + CritDmg/100)
    // Multiplier includes combat bonuses, enchants, weapon bonuses (1 without enchants)
    // ArmorBonus includes armor enchants (0 without reading armor)
    const initial = (5 + weaponDmg) * (1 + strength / 100);
    const base    = initial * multiplier * (1 + armorBonus);
        ...
        // ── /damage ──────────────────────────────────────────
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "🎯 Base Damage*", value: `Normal: **${fmt(base)}**\nCrit: **${fmt(crit)}**\nRating: ${damageRating(crit)}`, inline: true },
                    ...
                    { name: "\u200b", value: "\* Base damage only — enchant & armor bonuses not included yet." }
                )
                .setFooter({ text: "Dmg Bot • by VectorGOD19" });
            await interaction.reply({ embeds: [embed] });
        }
